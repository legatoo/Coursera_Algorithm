 ...
 * @author Kevin Wayne
 */
public class indexMinQ<Key extends Comparable<Key>> implements
		Iterable<Integer> {
	private int NMAX; // maximum number of elements on PQ
	private int N; // number of elements on PQ
	private int[] pq; // pq[i] = j; index: heap position, content: key index
	private int[] qp; // qp[i] = j; index: key index, content: heap position
	...
	public boolean contains(int i) {
		...
		return qp[i] != -1; //the heap position of key i
	}
	...
	private class HeapIterator implements Iterator<Integer> {
		// create a new pq
		private indexMinQ<Key> copy;

		// add all elements to copy of heap
		// takes linear time since already in heap order so no keys move
		public HeapIterator() {
			copy = new indexMinQ<Key>(pq.length - 1);
			for (int i = 1; i <= N; i++)
