...
public class minQueue<Key> implements Iterable<Key> {
     ...
     * @param comparator the order to use when comparing keys
     */
    public minQueue(Comparator<Key> comparator) { this(1, comparator); }
    ...
    **********************************************************************/
    ...
    private class HeapIterator implements Iterator<Key> {
        ...
        public HeapIterator() {
            if (comparator == null) copy = new minQueue<Key>(size());
            else                    copy = new minQueue<Key>(size(), comparator);
            for (int i = 1; i <= N; i++)
                copy.insert(pq[i]);
        }
